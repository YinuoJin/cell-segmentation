...
if __name__ == "__main__":
    ...
    out.sum().backward()
    """
    x_up = torch.rand(4, 256, 256, 256)
    x_e = torch.rand(4, 256, 256, 256)

    x_fwd = torch.stack([x_e, x_up], dim=0).to('cuda')
    x_rev = torch.stack([x_up, x_e], dim=0).to('cuda')

    cbgru = ConvBGRU(in_channels=256, hidden_channels=128, kernel_size=(3, 3), num_layers=1).to('cuda')
    out = cbgru(x_fwd, x_rev)
    out.sum().backward()
    y = out[:,1,...]
    print(x_up.shape, x_e.shape)
    print(x_fwd.shape, x_rev.shape)
    print(out.shape)
    print(y.shape)
