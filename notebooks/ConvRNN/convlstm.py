...
if __name__ == "__main__":
    x1 = torch.randn([4, 512, 256, 256]).cuda()
    x2 = torch.randn([4, 512, 256, 256]).cuda()
    ...
    print (x_fwd.shape, x_rev.shape, out.shape)
    # out.sum().backward()
    """

    clstm = ConvLSTM(in_channels=512, hidden_channels=512, kernel_size=(3, 3), num_layers=1, batch_first=True)
    x_in = torch.stack([x1, x2], axis=1)
    out = clstm(x_in)[0][0]
    print(out[:,-1,...].shape)
